...
# ------------------- Standardization ----------------
#########################################################
# Create a 5 dimmension dataset from df, just to check that
# Mean Shift part works fine:
D5_std_df = df[['customer_monetary_value',
                     'claims_rate',
                     'cancelled_premiums_pct',
                     'cust_pol_age', 
                     'premium_wage_ratio']]

Prm_std_df = df[['motor_premiums', 
                       'household_premiums', 
                       'health_premiums',
                       'life_premiums', 
                       'work_premiums']]
...
my_bandwidth = estimate_bandwidth(to_MS,
                                  quantile=0.2,    # Quantile of all the distances/ 0.3 is the default
                                  n_samples=1000)
...
from sklearn.decomposition import PCA
pca = PCA(n_components=2).fit(to_MS)    # fit to 2 principal components
...
from sklearn.decomposition import PCA
pca = PCA(n_components=3).fit(to_MS)    # fit to 3 principal components
